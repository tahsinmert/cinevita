...
export default function MovieDetail({ movie }: { movie: OmdbMovieFull }) {
  ...
  const isWatched = watchedActive || watchedHistory.includes(movie.imdbID)
  ...
  useEffect(() => {
    ...
    async function fetchRelated() {
      ...
      const genre = (movie.Genre || '').split(',')[0]?.trim() || 'movie'
      ...
      if (!cancelled) setRelated(recommendMovies(movie, details, 12))
      ...
      <section className="flex flex-col items-center gap-6">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="relative">
          ...
          {movie.Poster && movie.Poster !== 'N/A' ? (
            <Image width={500} height={500} src={movie.Poster} alt={movie.Title} className="h-64 w-64 rounded-full object-cover border border-neutral-200/60 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.25)]" />
          ) : (
            <div className="h-64 w-64 rounded-full bg-neutral-200 border border-neutral-200/60" />
          )}
          <div className="absolute -bottom-3 -right-3">
            <Button className="h-10 w-10 rounded-full" onClick={() => { add(movie.imdbID); toast.success('Added to Watchlist', { description: movie.Title }) }}>+</Button>
          </div>
        </motion.div>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          ...
          <div className="inline-flex items-center gap-1 relative">
            ...
            <div className="relative flex gap-1">
              <AnimatePresence>
                {typeof (storeRatings[movie.imdbID] ?? lastRated) === 'number' && (
                  <motion.span
                    key={`glow-${storeRatings[movie.imdbID] ?? lastRated}`}
              ...
              {[1,2,3,4,5].map((r) => (
                <motion.div key={r} whileHover={{ y: -1 }} whileTap={{ scale: 0.96 }}>
                  <Button
                    variant="outline"
                    className={((storeRatings[movie.imdbID] ?? lastRated) === r) ? 'border-emerald-500' : ''}
                    onClick={() => {
                      rate(movie.imdbID, r); setLastRated(r); toast.success(`Rated ${r}/5`)
                    }}
                  >{r}</Button>
                </motion.div>
              ))}
          ...
          <div className="relative">
            <motion.div whileHover={{ y: -1 }} whileTap={{ scale: 0.96 }}>
              <Button
                variant="outline"
                className={likedSet.has(movie.imdbID) ? 'bg-emerald-600 text-white border-emerald-600' : ''}
                onClick={() => {
                  if (!likedSet.has(movie.imdbID)) {
                    like(movie.imdbID); setLikeBurst((n) => n + 1); toast.success('You liked this')
                  }
                }}
              >
                {likedSet.has(movie.imdbID) ? 'Liked ✓' : 'Like'}
              </Button>
            </motion.div>
            <AnimatePresence>
              {[...Array(6)].map((_, i) => (
          ...
          <div className="relative">
            <motion.div whileHover={{ y: -1 }} whileTap={{ scale: 0.96 }}>
              <Button
                variant="destructive"
                className={dislikedSet.has(movie.imdbID) ? 'bg-red-600 text-white border-red-600' : ''}
                onClick={() => {
                  if (!dislikedSet.has(movie.imdbID)) {
                    dislike(movie.imdbID); setDislikeBurst((n) => n + 1); toast.message('Disliked')
                  }
                }}
              >
                {dislikedSet.has(movie.imdbID) ? 'Disliked ✕' : 'Dislike'}
              </Button>
            </motion.div>
            <AnimatePresence>
              {[...Array(6)].map((_, i) => (
